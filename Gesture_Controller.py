...
class HandRecog:
    ...
    def set_finger_state(self):
        ...
        # landmark information (x,y)coordinates
...
class GestureController:
    ...
    def __init__(self):
        ...
        GestureController.cap = cv2.VideoCapture(0)  # video object
    ...
    def start(self):
        ...
        with mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                success, image = GestureController.cap.read()  # will give frame

                if not success:
                    print("Ignoring empty camera frame.")
                    continue

                # send rgb image to object-"image"
                image = cv2.cvtColor(cv2.flip(image, 1), cv2.COLOR_BGR2RGB)
                image.flags.writeable = False
                results = hands.process(image)  # process frame and give result
                ...
                if results.multi_hand_landmarks:
                    ...
                    for hand_landmarks in results.multi_hand_landmarks:
                        # hand connections and 21 dots, line drawing
